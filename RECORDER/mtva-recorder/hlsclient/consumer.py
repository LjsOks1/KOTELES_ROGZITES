    ...
    def consume_resource(playlist_resource):
        ...
        try:
            ...
            if segments_modified:
                ...
                logging.info(segments_modified)
                balancer.notify_modified()
                balancer.m3u8_endlist(destination,segments_modified)
...
def download_to_file(uri, destination_path, current_key=None, new_key=False):
    ...
    FIXME: Modified to extract directory from segment filename.
    Assumed that filename has the structure of: CHANNEL_DATE_SEGMENT.
    '''
#    filename = os.path.join(destination_path, os.path.basename(uri))
    parts=string.split(os.path.basename(uri),'_')
    filename=os.path.join(destination_path,parts[1],os.path.basename(uri))
    if not os.path.exists(os.path.join(destination_path,parts[1])):
        os.makedirs(os.path.join(destination_path,parts[1]))        
    if not os.path.exists(filename):
        ...
        if not os.path.exists(os.path.join(destination_path,parts[1],"playlist.m3u8")):
            fd=open(os.path.join(destination_path,parts[1],"playlist.m3u8"),'w',1)
            fd.write("#EXTM3U\n")
            fd.write("#EXT-X-VERSION:3\n")
            fd.write("#EXT-X-TARGETDURATION:15\n")
            #FIXME:First sequence number should be extracted from uri!!!
            fd.write("#EXT-X-SEQUENCE:10001\n")
            fd.write("#EXT-X-PROGRAM-DATE-TIME:"+
                 datetime.datetime.today().strftime('%Y-%m-%dT%H:%M:%S')+"+01:00\n")
        else:
            fd=open(os.path.join(destination_path,parts[1],"playlist.m3u8"),'a',1)
        fd.write("#EXTINF:15.0,\n")
        fd.write(os.path.basename(uri)+'\n')
        fd.close()
        return filename
    else:
