...
class Balancer(object):
    ...
    def m3u8_endlist (self,destination_path,modified_segments):
        for segment in modified_segments:
            parts=string.split(os.path.basename(segment),"_")
            if self.last_segment:
                old_parts=string.split(os.path.basename(self.last_segment),"_")
                if old_parts[1] != parts[1]:  # We started to fetch segments for a different day! 
                    #Close the playlist for the old day...
                    logging.info("Closing playlist file in %s. Last segment:%s, New segment:%s",
                              destination_path,self.last_segment,segment)
                    fd=open(os.path.join(os.path.dirname(self.last_segment),"playlist.m3u8"),'a',1)
