...
#define DEFAULT_PROP_IGNORE_DURATION FALSE
...
/* LAJOS: Property to ignore duration. If true, subtitles arriving on text_sink 
*  overwrites the subtitle on screen. Used for vbi (teletext) subtitling, where 
*  we cannot wait for duration.
   Custom modification - not release. 
*  */ 
  g_object_class_install_property (G_OBJECT_CLASS (klass), PROP_IGNORE_DURATION,
      g_param_spec_boolean ("ignore-duration", "ignore duration",
          "Whether to ignore the duration timestamp of the buffer or wait until stop ts.",
          DEFAULT_PROP_IGNORE_DURATION,
          G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE | G_PARAM_STATIC_STRINGS));
    ...
    GstBaseTextOverlayClass * klass)
{
  ...
  overlay->ignore_duration = DEFAULT_PROP_IGNORE_DURATION;
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_VERTICAL_RENDER:
      ...
      overlay->need_render = TRUE;
      break;
    case PROP_IGNORE_DURATION:
      overlay->ignore_duration = g_value_get_boolean(value);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_OUTLINE_COLOR:
      ...
      break;
    case PROP_IGNORE_DURATION:
      g_value_set_boolean(value, overlay->ignore_duration);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
