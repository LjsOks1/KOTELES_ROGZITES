...
static GstStaticPadTemplate src_template = GST_STATIC_PAD_TEMPLATE ("src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
 //   GST_STATIC_CAPS (GST_VIDEO_CAPS_MAKE("RGBA") "; text/plain ; text/html")
    GST_STATIC_CAPS ("text/x-raw,format=utf8" )
    );

/* debug category for filtering log messages */
#define DEBUG_INIT(bla) \
  GST_DEBUG_CATEGORY_INIT (gst_teletextdec_debug, "teletextdec", 0, "Teletext decoder");

/* LAJOS: Original boilerplate
...
static gboolean gst_teletextdec_src_event (GstPad * pad, GstObject *parent, 
                                                GstEvent * event);
static gboolean gst_teletextdec_src_query(GstPad *pad, GstObject *parent,
                                                GstQuery *query);
...
gst_teletextdec_class_init (GstTeletextDecClass * klass)
{
  ...
  gst_teletextdec_base_init(gobject_class);
...
gst_teletextdec_init (GstTeletextDec * teletext)
{
  ...
  /* LAJOS: This should go to the event handler, but it is not defined for src pad.
  ...
  */
  ...
  gst_pad_set_query_function (teletext->srcpad,
      GST_DEBUG_FUNCPTR (gst_teletextdec_src_query));
...
gst_teletextdec_sink_event (GstPad * pad, GstObject *parent, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEGMENT:
      ...
      GST_INFO_OBJECT (teletext, "Segment event received!");
      gst_event_parse_segment(event,&teletext->segment);
    ...
    case GST_EVENT_CAPS: 
      {  
        ...
        gst_teletextdec_sink_setcaps(pad,caps);
  ...
  switch (GST_EVENT_TYPE (event)) {
   case GST_EVENT_CAPS: 
      {  
        GstCaps *caps;
        gst_event_parse_caps(event,&caps);
        gst_teletextdec_src_set_caps(pad,caps);
        ret=gst_pad_event_default(pad,parent, event);
        break;
      }	
    default:
...
static gboolean
gst_teletextdec_src_query(GstPad *pad, GstObject *parent, GstQuery *query)
{
...
gst_teletextdec_process_pes_buffer (GstTeletextDec * teletext, GstBuffer * buf)
{
  ...
  vbi_dvb_demux_feed (teletext->demux, map.data,map.size);
...
//      n_lines = teletext->frame->current_slice - teletext->frame->sliced_begin;
      n_lines = map.size / sizeof ( vbi_sliced);	
...
//          n_lines * sizeof (vbi_sliced));
      s = g_memdup (map.data , n_lines * sizeof(vbi_sliced));	
...
gst_teletextdec_push_page (GstTeletextDec * teletext)
{
  ...
  if (ret != GST_FLOW_OK)
    goto alloc_failed;
  GST_INFO_OBJECT (teletext,"Last timestamp (teletext->last_ts):%f" ,teletext->last_ts);
  GST_INFO_OBJECT (teletext, "Pushing buffer on src pad.Timestamp: %"GST_TIME_FORMAT
      ", Duration: %"GST_TIME_FORMAT,GST_BUFFER_TIMESTAMP(buf),GST_BUFFER_DURATION(buf));
...
{
  const gint line_length = page->columns+10;
  ...
  /* Print lines 2 to 23 */
  for (i = 1; i < 24; i++) {
    l=vbi_print_page_region (page, line, line_length + 1, "UTF-8", TRUE, 0,
        0, i, page->columns, 1);
    ...
    if (g_strcmp0 (line, "")) {
      ...
      GST_INFO_OBJECT(teletext, "Subtitle found in line %i : %s",i,line);
    }
  }
  ...
  *text = subs->str;
  GST_INFO_OBJECT(teletext,"Subtitle: %s",*text);
  if (subs->len >1) {
    length = subs->len ;
  }
  else {
    ...
    GstBuffer ** buf)
{
  ...
  /* LAJOS: Allocate new buffer - Original 
  ...
  *buf=gst_buffer_new_allocate(NULL,size,NULL);
  if( *buf !=NULL) {
    ...
    memcpy(bufdata,(guint8 *)text,size);
    clock=gst_element_get_clock(GST_ELEMENT(teletext));
    time=gst_clock_get_time(clock);
    time -=gst_element_get_base_time(GST_ELEMENT(teletext));
    gst_object_unref(clock);
    GST_BUFFER_PTS (*buf) = time;
    teletext->in_timestamp=time;
    ...
    GstBuffer ** buf)
{
  ...
  /* LAJOS: Allocate new buffer -- Original code
  ...
  *buf=gst_buffer_new_allocate(NULL,size,NULL);
  if( *buf !=NULL) {
    ...
    bufdata=map.data;
    memset(bufdata,0,size);
    memcpy(bufdata,(guint8 *)html,size);
    gst_buffer_unmap(*buf,&map);
    ret=GST_FLOW_OK;
  } else {
    ret=GST_FLOW_ERROR;
...
/* LAJOS: This was the original code
...
gst_teletextdec_push_preroll_buffer (GstTeletextDec * teletext)
{
  ...
  peer_caps = gst_pad_peer_query_caps (teletext->srcpad,NULL);
  pad_caps = gst_pad_query_caps (teletext->srcpad,NULL);
  ...
  buf = gst_buffer_new ();
//  gst_buffer_set_caps (buf, out_caps);   Not in gstreamer-1.0!!!
  ret = gst_pad_push (teletext->srcpad, buf);
  if (ret != GST_FLOW_OK)
    res = FALSE;
