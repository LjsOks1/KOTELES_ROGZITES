...
gst_hls_sink_class_init (GstHlsSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_INDEX,
      g_param_spec_uint ("index", "Start index value",
          "The index value of the first recorded file. Offset is used if recording starts "
          "after midnight. Available in customized hlssink. Experimental. ",
          0, G_MAXUINT, DEFAULT_INDEX,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
{
  sink->index = 0; //Used to name the files, also in m3u8 playlist (Can be resetted.)
  sink->count = 0; //Used to calculate the timestamps for the key units. (monotonic increased.)
  sink->index_offset = 0; 
  sink->timeout_id = 0;
...
gst_hls_sink_handle_message (GstBin * bin, GstMessage * message)
{
  ...
  switch (message->type) {
    case GST_MESSAGE_ELEMENT:
    {
      ...
      title = "Customized HLSSink";
      GST_INFO_OBJECT (sink, "COUNT %d, finished filename: %s", sink->count,filename);
      ...
      GST_INFO_OBJECT (sink,"m3u8 add entry: entry_location:%s, file:%s, offset:%d",
                entry_location, filename, sink->count+sink->index-sink->index_offset-1);
      gst_m3u8_playlist_add_entry (sink->playlist, entry_location, file,
          title, duration, sink->count+sink->index-sink->index_offset-1, discont);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PLAYLIST_LENGTH:
      ...
      break;
    case PROP_INDEX:
      sink->index=g_value_get_uint (value);
      sink->index_offset=sink->count;
      if ( sink->multifilesink) {
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PLAYLIST_LENGTH:
      ...
      break;
    case PROP_INDEX:
      g_value_set_uint (value, sink->index);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
    ...
    gpointer data)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM:
    {
      ...
      GST_INFO_OBJECT (sink, "setting index %d, timestamp:%"GST_TIME_FORMAT
               " stream_time:%"GST_TIME_FORMAT" running_time:%"GST_TIME_FORMAT"\n ", count,
               GST_TIME_ARGS(timestamp),GST_TIME_ARGS(stream_time),GST_TIME_ARGS(running_time));
//      sink->index = count;
      sink->count=count;
//      sink->index++;
      break;
...
//  running_time = sink->last_running_time + sink->target_duration * GST_SECOND;
  running_time= (sink->count+1)*sink->target_duration * GST_SECOND;
  GST_INFO_OBJECT (sink, "sending upstream force-key-unit, count %d "
      "now %" GST_TIME_FORMAT " target %" GST_TIME_FORMAT,
      sink->count + 1, GST_TIME_ARGS (sink->last_running_time),
      GST_TIME_ARGS (running_time));
  if ((sink->last_running_time > running_time) || 
      (sink->last_running_time + (2*sink->target_duration*GST_SECOND)<running_time)) {

      GST_WARNING_OBJECT (sink,"Too much difference. Current running time:%"GST_TIME_FORMAT " key event timestamp:%"GST_TIME_FORMAT"\n",
           GST_TIME_ARGS(sink->last_running_time),GST_TIME_ARGS(running_time));
  }
